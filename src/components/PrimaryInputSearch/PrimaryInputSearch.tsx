...
export const PrimaryInputSearch = () => {
  // const { accountUser, setAccountUser } = useContext(LolContext);
  ...
  return (
    ...
    >
      ...
      <button
        type="submit"
        className="w-full flex items-center justify-center py-2.5 px-5 me-2 text-sm font-medium text-gray-900 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:outline-none focus:ring-blue-700 focus:text-blue-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700"
      >
        {isLoading ? (
          <>
            <svg
      ...
      {errors.combinedInput && <span>This field is required</span>}
      {/* {resultData && (
          ...
          <div className="flex flex-col gap-2">
            {resultData && (
              <div className="text-white">
                {Object.keys(resultData).slice(0,5).map((key) => (
                  <div key={key}>
                    <strong>{key}:</strong>{" "}
